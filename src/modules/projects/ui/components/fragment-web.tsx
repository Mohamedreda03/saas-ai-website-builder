...
export default function FragmentWeb({ data }: FragmentWebProps) {
  ...
  const getSafeUrl = (url: string) => {
    ...
    if (url.startsWith("http://") && url.includes(".e2b.app")) {
  ...
  const onCopy = () => {
    ...
    navigator.clipboard.writeText(getSafeUrl(data.sandboxUrl));
    ...
    <div className="flex flex-col w-full h-full">
      <div className="p-2 border-b bg-sidebar flex items-center gap-x-2">
        ...
        <Hint text="Copy website URL" side="bottom" align="center">
          ...
          >
            <span className="truncate">{getSafeUrl(data.sandboxUrl)}</span>
          </Button>
        </Hint>
        ...
        <Hint text="Open in new tab" side="bottom" align="end">
          <Button
            ...
            onClick={() => {
              if (!data.sandboxUrl) return;
              window.open(getSafeUrl(data.sandboxUrl), "_blank");
            }}
          >
            <ExternalLinkIcon />
          </Button>
      ...
      <iframe
        ...
        loading="lazy"
        src={getSafeUrl(data.sandboxUrl)}
      />
    </div>
  );
}
